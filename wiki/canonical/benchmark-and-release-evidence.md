...
- Report schema v1: top-level fields should include `schema_version`, `benchmark_kind: "codex-actual-llm"`, `auth_mode`, auth audit metadata, `generated_at`, `environment`, `source_control`, `codex`, `configuration`, `summary`, `claim_gate`, and `scenarios`. Summary records `comparison_pair_count` so validators can prove selected scenarios form complete with/without pairs. Each scenario should include `scale`, `condition`, `task_family`, `prompt_id`, fixture fingerprint, `requested_model`, `model`, `model_source`, `models`, `runs`, claimable-run `median`, `median_all_runs`, claimable/all-run dispersion, `passed_run_count`, `claimable_run_count`, `correctness`, and `raw_jsonl_paths`. Each run records execution status, requested model, correctness, and measurement claimability. Validators must reparse raw JSONL paths and recompute run metrics, correctness, measurement claimability, comparison pair count, model source, medians, dispersion, and claim gate before accepting measured reports.
