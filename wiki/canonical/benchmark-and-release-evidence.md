...
- Scenario correctness validations must be recorded per measured run. Claimable median metrics use only runs that pass correctness and measurement claimability. Measurement claimability requires provider usage fields, positive token counts, positive wall-clock timing, final text, and exactly one model identified from JSONL or an explicit `--model` request; `median_all_runs` remains available for audit when any run fails, needs review, or lacks claimable measurement fields.
...
- Initial implementation includes dry-run fixture manifests, Codex JSONL parsing, expectation-based correctness checks, sample measured-report validation, gated measured execution through `benchmarks/codex-llm-metrics.js`, full selected scale/task matrix execution via `--full-matrix`, README-ready Markdown summary writing via `--markdown [path]`, public-claim gating via `--require-claimable`, clean-checkout gating via `--require-clean`, repeated-run minimum gating via `--min-runs-for-claim`, fixture/source-control provenance, dispersion metrics, and partial failure preservation. Measured Codex execution requires `--allow-codex-run` and defaults to one with/without pair to preserve comparison validity while limiting subscription quota use.
- Required actual LLM fields: `input_tokens`, `cached_input_tokens`, `output_tokens`, `reasoning_output_tokens` when available, `total_tokens` or derived total, wall-clock duration, first-token/first-response latency when JSONL timestamps expose it, tokens/sec, request/turn count, model, auth mode, and run timestamp.
...
- Required comparison dimensions: `scale` (`small`, `medium`, `large`), `condition` (`with_project_librarian`, `without_project_librarian`), `task_family`, `prompt_id`, `model`, `auth_mode`, token usage, wall-clock duration, first-token/first-response timing when available, Codex turn count, total JSONL item count, command/tool/plan call counts by event type, files read or referenced when observable, final answer correctness, and evidence-citation correctness.
...
- Matrix: every official report should cover `small`, `medium`, and `large` scales; `with_project_librarian` and `without_project_librarian` conditions; and task families `onboarding`, `decision_lookup`, `code_impact`, `release_policy`, and `change_location`. Use `--full-matrix` so the selected scales/tasks expand to every paired condition; any explicit `--max-scenarios` used with `--full-matrix` must equal the exact selected matrix size. Measured execution uses deterministic alternating pair order so the treatment condition is not always run before the control condition. Public claim candidates should also pass `--require-clean --require-claimable --min-runs-for-claim 3 --runs 3 --warmup-runs 1` unless quota constraints are explicitly documented.
- Fixture sizing: small means about 8-20 planning pages and 50-150 source/config files; medium means about 80-160 planning pages and 500-1,000 source/config files; large means 500+ planning pages and 1,500+ mixed source/config files with monorepo-shaped packages, workspace metadata, CODEOWNERS, and language variety.
- Treatment condition: bootstrap the fixture with Project Librarian and keep generated `AGENTS.md`, `.codex/hooks.json`, `.codex/hooks/wiki-session-start.js`, `wiki/startup.md`, `wiki/index.md`, canonical/decision/source/meta pages, scoped routers, and `.project-wiki/code-evidence.sqlite`.
...
- Measured command shape: `codex exec --json --ephemeral --sandbox read-only --skip-git-repo-check [--model <model>] "<prompt>"` from the generated fixture root. The runner must require explicit `--allow-codex-run` before launching Codex and should refuse measured subscription mode when `CODEX_API_KEY` or `OPENAI_API_KEY` is present unless `--auth-mode api-key` is explicitly requested.
- Raw artifacts: write JSONL under `benchmarks/reports/llm/raw/` and aggregate JSON/Markdown under `benchmarks/reports/llm/`. `--markdown` without a path writes the default current Markdown summary; `--markdown <path>` writes that explicit summary. Treat ad hoc LLM reports like other local benchmark reports: do not commit unless deliberately preserving release evidence.
- Report schema v1: top-level fields should include `schema_version`, `benchmark_kind: "codex-actual-llm"`, `auth_mode`, auth audit metadata, `generated_at`, `environment`, `source_control`, `codex`, `configuration`, `summary`, `claim_gate`, and `scenarios`. Summary records `comparison_pair_count` so validators can prove selected scenarios form complete with/without pairs. Each scenario should include `scale`, `condition`, `task_family`, `prompt_id`, exact `prompt`, exact `command`, fixture fingerprint, `requested_model`, `model`, `model_source`, `models`, `runs`, claimable-run `median`, `median_all_runs`, claimable/all-run dispersion, `passed_run_count`, `claimable_run_count`, `correctness`, and `raw_jsonl_paths`. Each run records execution status, requested model, correctness, and measurement claimability; failed executions are not claimable even when partial JSONL can be parsed. Validators must reparse raw JSONL paths and recompute run metrics, correctness, measurement claimability, comparison pair count, model source, medians, dispersion, selected matrix fingerprint, selected manifest fingerprint, and claim gate before accepting measured reports. The full fixture manifest fingerprint is retained separately as an audit field because partial measured reports cannot self-reconstruct unselected scenarios.
- Required parsed metrics: `input_tokens`, `cached_input_tokens`, `output_tokens`, `reasoning_output_tokens`, derived `total_tokens`, `wall_ms`, `first_response_ms` when available, `tokens_per_second`, `codex_turn_count`, `jsonl_event_count`, `command_event_count`, `command_invocation_count`, `tool_event_count`, `tool_invocation_count`, `mcp_event_count`, `mcp_invocation_count`, `plan_event_count`, `file_change_event_count`, `error_event_count`, model metadata, and `unavailable_event_fields`.
- Correctness checks: the current checker keeps task expectations in `benchmarks/lib/llm-correctness.js` and validates required terms, forbidden terms, task-specific expected evidence paths, read-only zero file changes, and missing final text. Validators must recalculate correctness from run metrics rather than trusting stored report status. Public improvement percentages should exclude `needs_review` cases unless manually adjudicated and documented.
- Implementation phases: dry-run, parser, sample measured-report validation, correctness checks, gated measured execution, paired with/without selection, claimable-run median gating, full selected scale/task matrix execution, Markdown summary generation, claim gate, provenance fingerprints, dispersion, first-response availability marking, partial failure reporting, stronger fixture realism, and stronger task-specific correctness are implemented. Remaining phases are further real-event taxonomy tuning when Codex JSONL adds or changes event shapes, optional expectation-file extraction, and README claim policy after a real measured report exists.
